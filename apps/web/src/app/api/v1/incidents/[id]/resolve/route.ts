...
import { assertCan, resolveIncidentRequestSchema } from '@leadops/core';
...
function mutationErrorResponse(error: unknown): Response | null {
  ...
  if (code === '23505' || message.includes('only open or acknowledged incidents can be resolved') || message.includes('status has changed')) {
  ...
  try {
    ...
    const result = await withTenantContext(
      ...
      async (tx) => {
        ...
        return resolveIncidentForTenant(txSql, {
          incidentId: id,
          organizationId: actor.organizationId,
          actor: actor.userId,
